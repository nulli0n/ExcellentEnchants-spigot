...
public class EnchantManager extends AbstractManager<EnchantsPlugin> {
    ...
    private void tickBlocks() {
        this.tickedBlocks.entrySet().removeIf(entry -> {
            TickedBlock tickedBlock = entry.getValue();
            this.plugin.runTask(entry.getKey(), tickedBlock::tick);
    ...
    public boolean createExplosion(@NotNull LivingEntity entity, @NotNull Location location, float power, boolean fire, boolean destroy, @NotNull Consumer<Explosion> consumer) {
        ...
        this.plugin.runTask(location, () -> entity.getWorld().createExplosion(location, power, fire, destroy, entity));
        return true;
    }
